...
def app(diabetes_df):
    ...
    glucose = st.slider('Glucose',min_value=40, max_value=200, step = 1)
    bp = st.slider('Blood pressure',min_value=40, max_value=100, step = 1)
    insulin = st.slider('Insulin',min_value=0, max_value=600, step = 1)
    height = st.slider('Height (in inch)',min_value=24, max_value=100, step = 1)
    weight = st.slider('Weight (in kg)',min_value=20, max_value=150, step = 1)
    pedigree = st.slider('Pedigree (Probability based on family history, kindly refer the internet for details and calculation)',min_value=0.0, max_value=3.0, step = 0.01)
